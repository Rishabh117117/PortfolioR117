...
/**
 * Follow — the system, in one picture (FOLLOW-SYS-1).
 *
 * The combined concept + how-it-works diagram: the team and the AI tools they
 * already use on top, the shared Follow index (an accent-colored slab)
 * underneath, chats flowing down and attributed answers flowing back up.
 * Four stations — team · capture · layer · recall — are real buttons: hover,
 * tap, or focus one and the caption dock explains it in plain terms with the
 ...
 * edges), GraphView's color language (facts accent, tools colored, files
 * gold), labels drawn from the sandbox's actual facts, plus the shipped
 * product's versioning story: spec v2 supersedes v1 and the old version
 * ghosts — kept, never deleted. A 16s CSS loop replays the whole life of a
 * fact (arrive → typed links → contradiction → supersession → query out); a
 * legend strip sits under the graph. Animations are scoped to .fsysOpen so
 * the loop restarts on every open; reduced motion gets the finished static
 * picture with a static caption.
 */

type Station = "team" | "capture" | "layer" | "recall";
...
> = {
  idle: {
    ...
    body: "Three teammates, three different AI tools, one shared memory between them. Hover — or tap — any part to see what it does, and click the index to look inside it.",
  },
  team: {
  ...
  recall: {
    ...
    body: "The next time anyone — or anyone’s AI — needs context, it asks Follow. The answer arrives attributed: who worked it out, in which chat, when.",
    eg: "e.g. Sam’s Gemini asks about wallet placement → Maya’s payment-sheet spec, cited back to her chat",
  },
};
...
const G_CONVS = [
  ...
  { x: 214, y: 304, c: GF.maya, label: "payment-sheet · thread", ly: 324 },
];
const G_FILES = [
...
const NEW = { x: 360, y: 208 }; // the arriving fact (falls down the centre lane)
const V1 = { x: 282, y: 338 }; // payment-sheet spec v1 — will be superseded
const V2 = { x: 392, y: 318 }; // spec v2 — arrives mid-loop
    ...
    >
      <div className="fsysGStage">
        <svg className="fsysG" viewBox="0 0 720 420" aria-hidden="true">
          ...
          <line className="fsysGEProd fsysGLinkA fsysGE3" pathLength={1} x1={V2.x} y1={V2.y} x2={214} y2={304} />
          ...
          {/* write packets in · query ring + read packet out */}
          <circle className="fsysGPktIn1" cx={NEW.x} cy="-16" r="5" />
          <circle className="fsysGPktIn2" cx={V2.x} cy="-16" r="5" />
        ...
        <div className="fsysGCaps" aria-hidden="true">
          ...
          <span className="fsysGCap fsysGCap4">spec v2 supersedes v1 — the old version ghosts, never deleted</span>
          <span className="fsysGCap fsysGCap5">a query pulls it back out — answer with author + thread attached</span>
          <span className="fsysGCap fsysGCap6">one graph — five edge types, every version kept</span>
          <span className="fsysGCapStatic">
            the team’s memory as a graph — typed edges, flagged contradictions, versions kept
          </span>
        </div>
      </div>
...
export default function FollowSystem() {
  ...
  return (
    <div
      ...
      data-active={active}
      role="group"
      aria-label="How Follow works — an interactive diagram. Three teammates and their AI tools sit above the shared Follow index; chats flow in, attributed answers flow back. The index opens to show the knowledge graph inside."
    >
